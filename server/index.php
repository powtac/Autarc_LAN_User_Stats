...
if (isset($_GET['AVR_ID'])) {
    $GET_AVR_ID = mysql_real_escape_string($_GET['AVR_ID']);
...
';
	...
	$output .= '-->';
        ...
        $online_list = mysql_query("SELECT MAC, IP, time_offline, last_scan FROM Online_DB WHERE AVR_ID='".$GET_AVR_ID."' ORDER BY IP ASC");
...
else if (isset($JSON["AVR_ID"])) {
	/* New online data */

	$stored = false;
	$AVR_ID = mysql_real_escape_string($JSON["AVR_ID"]);
